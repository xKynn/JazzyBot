...
class Playlist(Command):
    name = "playlist"
    alts = ["pl", "mylist"]
    oneliner = "Save songs on the bot so you can listen to them anytime quickly, a personal playlist!"

    help = "You can save/play a whole queue with the playlist\nYou can also play a range of tracks from your playlist "\
           "by using `<prefix>playlist play start_number-end_number`"

    examples = "<prefix>playlist [optional page no.]- shows your playlist\n<prefix>playlist add [song_number or 'np'] "\
               "- adds the song from current queue with the song number or now playing song if 'np' is " \
               "used\n<prefix>playlist dump - whole current queue is added to your playlist\n<prefix>playlist play [" \
               "optional song number or range] - plays the song number from your playlist, if nothing is specified " \
               "whole playlist is played\n<prefix>playlist reorder number>number - swap the places of the two numbers "\
               "specified in your playlist\n<prefix>playlist delete [song_number] - deletes the song with that number "\
    ...
    async def main(bot, message):
        ...
        printlines = {0: []}
        d = bot.ses
        if len(message.content.split()) == 1 or not message.content.split()[1] in ['dump', 'add', 'clear', 'play',
                                                                                   'reorder']:
            ...
            if pg_index < 1 or pg_index > total_pgs:
                await SMexHandler.handle(bot, ServerManPrettyException(
                    "Your playlist only has %s page(s)!\nPlease a supply a value between 1 and %s." % (
                        total_pgs, total_pgs), "Invalid Value!", message.channel))
                return
            ...
            if len(song_index) > 3:
                await message.channel.send(
                    "I never thought someone would make a playlist so big, but here you are, I just made a precautionary block for above 100, but well i might have to get rid of this, thanks, %s" % message.author.name)
                return
            if song_index == ('np'):
            ...
            if '-' in message.content.split()[2].lower():
                ...
                for play_index in range(from_index, to_index):
                    try:
                        info = await bot.downloader.extract_info(bot.loop, track_urls[play_index - 1], download=False,
                                                                 process=False, retry_on_error=True)
                        entry, position = player.playlist.add(info['webpage_url'], message.author, message.channel,
                ...
                await message.channel.send(
                    ":notes: Queued songs from **%s-%s**, from your saved playlist" % (from_index, to_index))
                ...
                await message.channel.send(
                    ":notes: Playing **%s**, from your saved playlist" % track_names[play_index - 1])
                info = await bot.downloader.extract_info(bot.loop, track_urls[play_index - 1], download=False,
                                                         process=False, retry_on_error=True)
                entry, position = player.playlist.add(info['webpage_url'], message.author, message.channel,
                                                      info['title'], info['duration'])
                ...
                for track in track_urls:
                    info = await bot.downloader.extract_info(bot.loop, track, download=False, process=False,
                                                             retry_on_error=True)
                    entry, position = player.playlist.add(info['webpage_url'], message.author, message.channel,
                                                          info['title'], info['duration'])
                    if player.state == 'stopped':
