...
class Jump(Command):
    ...
    examples = "`<prefix>jump 2` - If the bot was at 17 before, it will jump after the current song finishes playing " \
               "and continue playing from 2, i.e 2, 3, 4.. "
    options = "None"
    ...
    async def main(bot, message):
        ...
        if pickno - 1 < 0 or pickno > len(player.playlist.entries):
            await SMexHandler.handle(bot, ServerManPrettyException(
                "Value can only be between 1 and %s" % len(player.playlist.entries), "Invalid Value!", message.channel))
            return
