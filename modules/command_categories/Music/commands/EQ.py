...
class EQ(Command):
    ...
    help = "Once you've applied an Equalizer, it'll stay applied till the bot leaves your voice " \
    ...
    async def main(bot, message):
        effects = {'pop': 'Pop', 'classic': 'Classic', 'jazz': 'Jazz', 'rock': 'Rock', 'bb': 'Bass Boost',
        ...
        if not eq.lower() in effects.keys():
            await SMexHandler.handle(bot, ServerManPrettyException("%s, is not a valid EQ effect." % eq,
                                                                   "Invalid EQ effect!", message.channel))
            return
        player.EQ = eq.lower()
        ...
        await send(bot, "Equalizer", ":loud_sound: Equalizer has been set to %s." % effects[eq.lower()], 0xCCD6DD,
                   message.channel)
