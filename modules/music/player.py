...
class Player:
    def __init__(self,bot,voice_client, playlist):
        ...
        self.EQEffects = {'normal':"",'pop': ' -af equalizer=f=500:width_type=h:w=300:g=2,equalizer=f=1000:width_type=h:w=100:g=3,equalizer=f=2000:width_type=h:w=100:g=-2,equalizer=f=4000:width_type=h:w=100:g=-4,equalizer=f=8000:width_type=h:w=100:g=-4,equalizer=f=16000:width_type=h:w=100:g=-4',
            ...
            if not self.repeat:
                if ind == None:
                    if self.state in ['stopped','switching']:
                        if not self.current_player == None and self.voice_client.is_playing():
                            return
                ...
                with await entry['lock']:
                    ...
                    if 'status' in entry.keys():
                        ...
                        #If an entry is currently being downloaded, the status is 'processing', so we don't bother with it and return
            ...
            with await now['lock']:
                ...
                # is guessed as mono and Voila! Karaoke
                    ...
                    if not 'karaoke_'+now['filename'].split(self.slash)[1] in onlyfiles:
                        ...
                        p1 = subprocess.Popen(['ffmpeg', '-i',now['filename'],'-af','pan=stereo|c0=c0|c1=-1*c1','-ac','1', now['filename'].split(self.slash)[0] + self.slash + 'karaoke_'+now['filename'].split(self.slash)[1].split('.')[0]+'.wav'])
                        p1.wait()
                        procm.edit(content="Done! :white_check_mark:")
                    now['filename'] = now['filename'].split(self.slash)[0] + self.slash + 'karaoke_'+now['filename'].split(self.slash)[1].split('.')[0]+'.wav'
    ...
    async def manage_nowplaying(self):
        ...
        np_embed = discord.Embed(title =player.current_entry['title'],  description = 'added by **%s**' % player.current_entry['author'].name, url = player.current_entry['url'], colour=0xffffff )
            ...
            while 1:
                ...
                try:
                    for entry in self.playlist.entries:
                        if entry['url'].split('/')[3] == altitems[song_choice].select('a[href^=/watch]')[0].attrs.get('href').split('/')[1]:
                            test = True
                            break
                except:
                    test = False
                ...
                if len(altitems[song_choice].select('.yt-badge-live')) or test:
                    ...
                    song_url = 'http://www.youtube.com' + altitems[song_choice].select('a[href^=/watch]')[0].attrs.get('href')
                    break
